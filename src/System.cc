...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(settings_ && settings_->needToRectify()){
        // cout << "INFO: rectify stereo image pair." << endl;
    ...
    // cout << "[Tracking]: TrackingState: "<< mTrackingState << ", Tracked MPs: " <<mTrackedMapPoints.size()  << ", Tracked KPs: " <<mTrackedKeyPointsUn.size() << endl;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            ...
            Eigen::Quaternionf q = Twb.unit_quaternion();
            Eigen::Vector3f twb = Twb.translation();
            f << setprecision(9) << pKF->mTimeStamp  << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        ...
        else
        {
            ...
            Eigen::Quaternionf q = Twc.unit_quaternion();
            Eigen::Vector3f t = Twc.translation();
            f << setprecision(9) << pKF->mTimeStamp << " " <<  setprecision(9) << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
    }
    f.close();
}
...
// (cj) Can be possibly removed, since it is designed for multiple maps
...
// (cj) save the maps to local for later usage. Ignore it for now

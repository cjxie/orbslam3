...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mnWidth = settings->newImSize().width;
    mnHeight = settings->newImSize().height;
        ...
        if(mCurrentFrame.mvpMapPoints[i])
        {
            if(!mCurrentFrame.mvbOutlier[i])
            {
                ...
                if(!mbOnlyTracking)
                {
                    ...
                    {
                        mnMatchesInliers++;
                        if (mCurrentFrame.mvpMapPoints[i]->mTrackProjY > mnHeight / 2.0)
                            mnMatchesOnGrass++;
                    }
